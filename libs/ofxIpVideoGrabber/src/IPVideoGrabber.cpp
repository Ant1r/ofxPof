...
void IPVideoGrabber::update()
{
    ...
    uint64_t now = ofGetSystemTimeMicros()/1000;
...
uint64_t IPVideoGrabber::getTimeTillNextAutoRetry() const
{
    ...
    auto now = ofGetSystemTimeMicros()/1000;
    ...
    {
        return nextAutoRetry_a - ofGetSystemTimeMicros()/1000;
    }
}
...
void IPVideoGrabber::threadedFunction()
{
    ...
    connectTime_a = ofGetSystemTimeMicros()/1000; // start time
    nextAutoRetry_a = 0;
    ...
    catch (const Poco::Exception& e)
    {
        ...
        needsReconnect_a = true;
//        _isConnected = false;
        nextAutoRetry_a = ofGetSystemTimeMicros()/1000 + autoRetryDelay_a;
        mutex.unlock();
